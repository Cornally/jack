...
IMPROVEMENTS:
  ...
  * Stubbed out .field__helper element styling to form fields that have helper states for describing an input's status.  A common use case would be indicating a required field has not been filled out when the field loses focus or the form is submitted.

BUG FIXES:

  * Corrected type utility class names.

BREAKING CHANGES:

  * Classes generated by `generateColorTicks` and `generateFontColor` mixins now use a `-` spacing character instead of `_`.  Placeholders retain their `_` spacing character.
  * Updated `generateColorTicks` and `generateFontColor` mixin 'name' property to automatically insert the correct spacing character based on whether or not `'class'` or `'placeholder'` is passed as the output type.
  * Leading `0` removed from `generateColorTicks` and `generateFontColor` mixin output placeholders and classes.
